        ...
        @if(!empty($subject))
            @foreach($subject as $key => $row)
                <tr>
                    ...
                    <td>
                        <button class="btn btn-primary mt-2 mt-xl-0 text-right register-courses" data-name_teacher="{!! $row->join_teacher->name_teacher !!}{!! $row->join_teacher->lastname_teacher !!}" data-stop="{!! $row->time_stop !!}" data-start="{!! $row->time_start !!}"  data-id="{!! $row->id_subject !!}" data-name="{!! $row->{'name_subject_'.Session::get('locale')}  !!}" ><i class="mdi mdi-spellcheck"></i></button>
                    </td>
                </tr>
            @endforeach
        @else
