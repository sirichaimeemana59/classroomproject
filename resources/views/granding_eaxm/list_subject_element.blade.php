        ...
        @if(!empty($subject))
            @foreach($subject as $key => $row)
                <tr>
                    ...
                    <td>
                        @if($row->join_examination)
                            <a href="{!! url('/teacher/grading_the_exam/list_student/'.$row->id_subject) !!}"><button class="btn btn-warning mt-2 mt-xl-0 text-right"><i class="mdi mdi-format-list-bulleted-type"></i></button></a>
                        @else
                            <button class="btn btn-warning mt-2 mt-xl-0 text-right" disabled><i class="mdi mdi-format-list-bulleted-type"></i></button>
                        @endif
                        {{--<button class="btn btn-primary mt-2 mt-xl-0 text-right add-eam" data-id="{!! $row->id_subject !!}"><i class="mdi mdi-format-list-bulleted-type"></i></button>--}}
